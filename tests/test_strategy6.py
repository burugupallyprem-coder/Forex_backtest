"""Strategy #6 (wide basket + swap) correctness. Run: python tests/test_strategy6.py"""
import sys
...
from src.backtest.strategy6_regime_wide import (efficiency_ratio, pip_size,
...
def test_swap_reduces_pnl():
    # same winning long trade; a large swap drag must lower net PnL and R
    df = daily(UPTREND)
    p = {"er_min": 0.5}
    t0 = simulate_instrument(df, "EUR_USD", p, {**CFG, "swap_annual_pct": 0.0})
    t1 = simulate_instrument(df, "EUR_USD", p, {**CFG, "swap_annual_pct": 50.0})
    assert t0 and t1
    assert t1[0].pnl < t0[0].pnl and t1[0].r_multiple < t0[0].r_multiple
